...
class Cube (object):
    def __init__(self,a=10,b=10,c=10,x=100):
        self.a = a
        self.b = b
        self.c = c
        self.x = x
        self.o = Vector3(0,0,0)
        self.pts = [ Vector3(-a,b,c)  , Vector3(a,b,c)
                   , Vector3(a,-b,c)  , Vector3(-a,-b,c)
                   , Vector3(-a,b,-c) , Vector3(a,b,-c)
                   , Vector3(a,-b,-c) , Vector3(-a,-b,-c) ] 
        self.axis = [Vector3(x,0,0), Vector3(0,x,0), Vector3(0,0,x)]
    ...
    def erase(self,screen):
        ...
        edges = self.edges()
        axes = self.axes()
        erasables = sides + edges + axes
        [ s.erase(screen) for s in erasables]
        
    def erase2D(self, screen):
        assert isinstance(screen, Screen)
        edges = self.edges()
        axes = self.axes()
        erasables = axes+edges
        [s.erase(screen) for s in erasables]
    ...
    def axes(self):
        ...
        edges = [Edge(ao,ax,xc), Edge(ao,ay,yc), Edge(ao,az,zc)]
        return edges
    ...
    def draw(self,screen,q=Quaternion(1,0,0,0)):
        ...
        edges = self.edges()
        axes = self.axes()
        drawables = sides + edges + axes
        drawables.sort(key=lambda s: screen.depth(s.centroid()))
        [ s.draw(screen) for s in drawables ]
        
    def draw2D(self, screen, q=Quaternion(1,0,0,0), px=0, py=0):
        assert isinstance(screen, Screen)
        self.origin()
        self.rotate(q)
        self.translate(px, py, 0)
        #sides = self.sides()
        edges = self.edges()
        axes = self.axes()
        drawables = axes+edges
        #drawables.sort(key=lambda s:screen.depth(s.centroid()))
        [s.draw(screen) for s in drawables]
        
    def draw3D(self,screen,q=Quaternion(1,0,0,0),px=0,py=0,pz=0):
        """ draw object at given rotation """
        assert isinstance(screen,Screen)
        self.origin()
        self.rotate(q)
        self.translate(px,py,pz)
        sides = self.sides()
        edges = self.edges()
        axes = self.axes()
        drawables = sides + edges + axes
    ...
    def rotate(self,q):
        ...
        self.axis = [R*a for a in self.axis]

    def translate(self,px,py,pz):
        trans = Vector3(px,py,pz)
        self.o += trans
        self.pts = [trans+p for p in self.pts]
        self.axis = [trans+a for a in self.axis]
